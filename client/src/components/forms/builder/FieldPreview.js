...
const FieldPreview = ({ field }) => {
  ...
  switch (field.type) {
    ...
    case 'dropdown':
      return (
        <select className={inputClass} disabled>
          ...
          {(field.options || []).map((opt, i) => <option key={i}>{opt.label || `Option ${i + 1}`}</option>)}
        </select>
      );
    ...
    case 'file':
      return (
        <div className="px-4 py-6 border-2 border-dashed border-gray-300 rounded-lg text-center text-gray-400 text-sm">
